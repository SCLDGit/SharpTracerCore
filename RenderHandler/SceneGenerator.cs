...
namespace RenderHandler
{
    ...
    {
        public SceneGenerator(List<IHitTarget> p_worldHitTargets, ICamera p_camera)
        {
            World = new World(p_worldHitTargets);
            Camera = p_camera;
        }
        ...
        internal static SceneGenerator GenerateRandomMovingScene(RenderParameters p_renderParameters, int p_numberOfObjects)
        {
            var lookFrom = new Vec3(-13, 2, -3);
            var lookAt = new Vec3(0, 0, 0);
            const double focalDistance = 10.0;
